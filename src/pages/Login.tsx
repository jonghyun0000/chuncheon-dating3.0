...
export default function Login() {
  ...
  // user 상태가 바뀔 때마다 리디렉트 체크
  useEffect(() => {
    if (authLoading) return
    if (!user) return

    console.log('로그인된 user role:', user.role) // 디버그용
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 성공 시 useEffect가 user 변경 감지해서 자동 리디렉트
  }
    ...
    <div style={S.page}>
      ...
      <main style={S.card} className="fade-up">
        ...
        <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="field fade-up fade-up-2">
            ...
            <input
              ...
              autoComplete="username" autoFocus maxLength={20} disabled={loading}
            />
          </div>
          ...
          <button
            type="submit" className="btn-primary fade-up fade-up-4"
            disabled={loading || !username || !password}
            style={{ marginTop: 4 }}
          >
            {loading
              ? <span className="spinner" style={{ margin: '0 auto', width: 20, height: 20, borderWidth: 2 }} />
              : '로그인'}
          </button>
        </form>
        ...
        <p style={{ textAlign: 'center', fontSize: 11, color: 'var(--color-text-hint)', marginTop: 16, lineHeight: 1.6 }}>
          로그인 시 <Link to="/terms" style={{ color: 'var(--color-text-muted)', textDecoration: 'underline' }}>이용약관</Link> 및{' '}
          <Link to="/privacy" style={{ color: 'var(--color-text-muted)', textDecoration: 'underline' }}>개인정보처리방침</Link>에 동의한 것으로 간주합니다.
        </p>
      </main>
    </div>
  )
